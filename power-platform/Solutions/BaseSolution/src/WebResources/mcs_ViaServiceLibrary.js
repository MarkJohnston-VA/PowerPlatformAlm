...
function getConfig() {
    ...
    //https://va-veis-nprod-apim.nprod.veis.va.gov/EC/VIAEMRService/api/VEISEMRmatch
    ...
    if (apimUrl == "" || subKeys == "") {
        ...
        CommCare.Shared.CrmCommonJS.WebApi.RetrieveMultiple("mcs_settings", columns, filter).then(function (results) {
            if (results.value != null) {
                apimUrl = data.value[0].mcs_dacurl;
                subKeys = "Ocp-Apim-Subscription-Key|" + data.value[0].mcs_subscriptionid_east;
            }
...
function vialib_getApimToken() {
    vialib_callAction("mcs_GetAPIMToken", { },
	function (data) {
	    //	
	    apimToken = data.token;
...
function vialib_getNoteTitles(vialib_requestingApp, vialib_consumingAppToken, vialib_consumingAppPassword, vialib_baseServiceEndpointUrl, vialib_providerName, vialib_Duz, vialib_loginSiteCode, vialib_Target, vialib_Direction, vialib_responseCallback, vialib_externalObject) {
    ...
    try {
        ...
        $.ajax({
            ...
            datatype: "json",
            beforeSend: function (xhr) {
                xhr.setRequestHeader("Authorization", "Bearer " + apimToken);
                var keys = subKeys.split("|");
                for (var i = 0; i < keys.length; i = i + 2) {
                    xhr.setRequestHeader(keys[i], keys[i + 1]);
                }
            ...
            contentType: "application/json",
            success: function (data) {
                HandleNoteTitles(null, data.noteTitles.results, null);
...
function vialib_getLocations(vialib_requestingApp, vialib_consumingAppToken, vialib_consumingAppPassword, vialib_baseServiceEndpointUrl, vialib_providerName, vialib_Duz, vialib_loginSiteCode, vialib_Target, vialib_Direction, vialib_responseCallback, vialib_externalObject) {
    ...
    try {
        ...
        var data = {
            "queryBean": {
                "requestingApp": vialib_encodeString(CommCare.Shared.VIAConsumingApplication),
                "consumingAppToken": vialib_encodeString(CommCare.Shared.VIAConsumingAppToken),
                "consumingAppPassword": vialib_encodeString(CommCare.Shared.VIAConsumingAppPassword),
                "provider": {
                    "name": vialib_encodeString(vialib_providerName),
                    "loginSiteCode": vialib_loginSiteCode,
                    "userId": vialib_encodeString(vialib_Duz)
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                HandleLocations(null, data.locations.resultsInfo, null);
            },
            error: function (jqXHR, textStatus, errorThrown) {
                //System Error
                var response = JSON.parse(jqXHR.response);
