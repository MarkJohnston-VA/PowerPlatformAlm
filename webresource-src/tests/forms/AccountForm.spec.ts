import { AccountForm } from '../../src/forms/AccountForm';
...
describe('AccountForm', () => {
  describe('OnLoad', () => {
    it('should call the base OnLoad method', async () => {
      ...
      var instance = new AccountForm();

      //Act
      await instance.OnLoad(eventContext);

      //Assert
      expect(setFormNotificationSpy).toHaveBeenCalledWith('Test1 - OnLoad handler has fired', 'WARNING', 'OnLoadHandler');
      expect(setDisabledSpy).toHaveBeenCalledWith(true);
      expect(addOnChangeSpy).toHaveBeenCalledWith(instance.Name_OnChange);
    });
  });

  describe('Name_OnChange', () => {
    it('should clear notification and log new value', () => {
      ...
      const setFormNotificationSpy = jasmine.createSpy('setFormNotification').and.returnValue('Name_OnChange');
      eventContext.getFormContext = () => ({
        getAttribute: getAttributeSpy,
        log: () => {},
        ui: {
      ...
      // Act
      instance.Name_OnChange(eventContext);
      ...
      expect(setFormNotificationSpy).toHaveBeenCalledWith('Name_OnChange has fired. New Value: New Value', 'INFO', 'Name_OnChange');
    });
  });
});