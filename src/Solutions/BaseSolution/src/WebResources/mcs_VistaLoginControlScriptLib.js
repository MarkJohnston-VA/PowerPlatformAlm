...
function GetSettingsCallback(results) {
    try {
        ...
        for (var i = 0; i <= results.length - 1; i++) {
            if (results[i].mcs_dacurl != null) { vlc_dacurl = results[i].mcs_dacurl; }
            if (results[i].mcs_vialoginurl != null) { vlc_vialoginapiurl = results[i].mcs_vialoginurl; }
            if (results[i].mcs_vialoginminutes != null) { vlc_vialoginminutes = results[i].mcs_vialoginminutes; }
            if (results[i].mcs_viassoiloginurl != null) { vlc_viassoiloginapiurl = results[i].mcs_viassoiloginurl; }
            if (results[i].mcs_viassoirefreshtokenurl != null) { vlc_viassoirefreshtokenurl = results[i].mcs_viassoirefreshtokenurl; }
            if (results[i].mcs_viassoipivcardurl != null) { vlc_viassoipivcardurl = results[i].mcs_viassoipivcardurl; }

            if (results[i].mcs_viaservicebaseurl != null) { vlc_baseserviceendpointurl = results[i].mcs_viaservicebaseurl; }
            if (results[i].mcs_viarequestingapplicationcode != null) { vlc_consumingApp = results[i].mcs_viarequestingapplicationcode; }
            if (results[i].mcs_viaconsumingapplicationtoken != null) { vlc_consumingAppToken = results[i].mcs_viaconsumingapplicationtoken; }
            if (results[i].mcs_viaconsumingapplicationpassword != null) { vlc_consumingAppPassword = results[i].mcs_viaconsumingapplicationpassword; }
            if (results[i].mcs_mvisubscriptionkey != null) { vlc_mvisubscriptionkey = results[i].mcs_mvisubscriptionkey; }
            break;
        }
...
function vlc_formLoad() {
    ...
    try {
        ...
        CommCare.Shared.CrmCommonJS.WebApi.RetrieveMultiple("mcs_settings", columns, filter).then(function (results) {
            GetSettingsCallback(results.value);

            var columns = "_mcs_visnid_value, _mcs_facilitysiteid_value";
            ...
            CommCare.Shared.CrmCommonJS.WebApi.RetrieveRecord(userId, "systemusers", columns).then(function (user) {
                if (user != null) {
                    visnId = user._mcs_visnid_value;
                    if (visnId != null) {
                        if (visnId[0] == '{')
                        ...
                        var filter = "$filter=_bah_visnid_value eq " + visnId + " and statecode eq 0";
                        ...
                        CommCare.Shared.CrmCommonJS.WebApi.RetrieveMultiple("bah_facilities", columns, filter).then(function (results) {
                            ...
                            if (results.value != null) {
                                ...
                                for (var i = 0; i < results.value.length; i++) {
                                    ...
                                    var ftp_name = results.value[i]["bah_name"];

                                    if (ftp_facilityid != null) {
                                        vlc_userSiteId = ftp_facilityid;
                                    }

                                    if (ftp_facilitycode_text != null) {
                                        vlc_UserSiteNo = ftp_facilitycode_text;
                                    }
                                    ...
                                    cell11.innerHTML = "<div id=\"BusySpinner" + curRow.toString() + "\" style=\"display:none\"><img src=\"mcs_BusySpinner\"></div>";
                    ...
                    //vlc_getMultipleEntityDataAsync('mcs_settingSet', 'mcs_DACURL, mcs_VIALoginURL, mcs_VIALoginMinutes, mcs_VIASSOiLoginURL, mcs_VIASSOiRefreshTokenURL, mcs_VIASSOiPIVCardURL, mcs_VIAServiceBaseURL, mcs_VIARequestingApplicationCode, mcs_VIAConsumingApplicationToken, mcs_VIAConsumingApplicationPassword, mcs_MVISubscriptionKey', vlc_conditionalFilter, 'mcs_name', 'asc', 0, vlc_SettingsWebServiceURL_response, vlc_ViaLoginUrl);
                    if (vlc_USDdata != "USD") {
                    ...
                    // Moved to callback above to avoid additional call
                    //Get the current CRM User's assigned site/facility
                    //var vlc_userData = vlc_getSingleEntityDataSync('SystemUserSet', 'mcs_FacilitySiteId', vlc_userId);
                    //if (vlc_userData != null) {
                    //    if (vlc_userData.d.mcs_FacilitySiteId != null) {
                    //        vlc_userSiteId = vlc_userData.d.mcs_FacilitySiteId.Id;
                    //    }
                    //}
                    ...
                    //    var vlc_facilityData = vlc_getSingleEntityDataSync('bah_facilitySet', 'bah_stationsuffix_text', vlc_userSiteId);
                    //    if (vlc_facilityData != null) {
                    //        //if (vlc_facilityData.d.ftp_facilitycode != null) { vlc_UserSiteNo = vlc_facilityData.d.ftp_facilitycode; }
                    //        if (vlc_facilityData.d.bah_stationsuffix_text != null) { vlc_UserSiteNo = vlc_facilityData.d.bah_stationsuffix_text; }
                    //    }
                    //}
                }
...
function vlc_vistaLogin(row, facilityName, facilityId) {
    ...
    try {
        ...
        vlc_viaLogin.SiteId = facilityName;
        vlc_viaLogin.Target = "";

        var vlc_loginResponse = "";

        var parameters = {};
        parameters.Request = window.JSON.stringify(vlc_viaLogin);

        var GetVistaLogin = CommCare.Shared.CallAction("mcs_VistaLogin", parameters);
        var VistaLogin = GetVistaLogin.then(function (result) {
            if ((result != null) && (result.Response != null)) {
                vlc_loginResponse = JSON.parse(result.Response);
    ...
    try {
        ...
        if (vlc_loginResponse.ErrorOccurred == true) {
            ...
            CommCare.Shared.CrmCommonJS.WebApi.RetrieveMultiple("mcs_settings", columns, filter).then(function (results) {
                ...
                if (results.value != null) {
                    for (var i = 0; i <= results.value.length - 1; i++) {
                        if (results.value[i].mcs_vialoginminutes != null) { expirationminutes = results.value[i].mcs_vialoginminutes; }
                        break;
                    }
                }
                ...
                vlc_setCookie("viasessionlink", JSON.stringify(vlc_CookieData), expirationminutes);
                var vlc_ViaLoginCookie = vlc_getCookie("viasessionlink");
                //parent.Xrm.Page.ui.setFormNotification("ERROR: YOUR VISTA SESSION HAS EXPIRED, PLEASE LOGIN BELOW!", "ERROR", "VIASERVICE");
                parent.Xrm.Page.ui.clearFormNotification("VIASERVICE");
                //viawl_onLoad(vlc_ViaLoginCookie);
...
function vlc_setCookie(cname, cvalue, exminutes) {
    try {
        ...
        var intMins = parseInt(exminutes);
        var newmins = mins + intMins;
        d.setMinutes(newmins);
...
//    req.open("GET", Xrm.Page.context.getClientUrl() + "/api/data/v8.2/systemusers(" + userId + ")?$select=_mcs_visnid_value", false);
...
//                var _mcs_visnid_value = result["_mcs_visnid_value"];
//                var _mcs_visnid_value_formatted = result["_mcs_visnid_value@OData.Community.Display.V1.FormattedValue"];
//                var _mcs_visnid_value_lookuplogicalname = result["_mcs_visnid_value@Microsoft.Dynamics.CRM.lookuplogicalname"];
//                visnId = _mcs_visnid_value;
//            } else {
//                Xrm.Utility.alertDialog(this.statusText);
//            }
//        }
...
//            //Get info
//            if (vlc_settingData.d.results[i].mcs_DACURL != null) { vlc_DacUrl = vlc_settingData.d.results[i].mcs_DACURL; }
//            if (vlc_settingData.d.results[i].mcs_VIALoginURL != null) { vlc_ViaLoginApiUrl = vlc_settingData.d.results[i].mcs_VIALoginURL; }
//            if (vlc_settingData.d.results[i].mcs_VIALoginMinutes != null) { vlc_ViaLoginMinutes = vlc_settingData.d.results[i].mcs_VIALoginMinutes; }
//            if (vlc_settingData.d.results[i].mcs_VIASSOiLoginURL != null) { vlc_ViaSSOiLoginApiUrl = vlc_settingData.d.results[i].mcs_VIASSOiLoginURL; }
//            if (vlc_settingData.d.results[i].mcs_VIASSOiRefreshTokenURL != null) { vlc_ViaSSOiRefreshTokenUrl = vlc_settingData.d.results[i].mcs_VIASSOiRefreshTokenURL; }
//            if (vlc_settingData.d.results[i].mcs_VIASSOiPIVCardURL != null) { vlc_ViaSSOiPivCardUrl = vlc_settingData.d.results[i].mcs_VIASSOiPIVCardURL; }

//            if (vlc_settingData.d.results[i].mcs_VIAServiceBaseURL != null) { vlc_baseServiceEndpointUrl = vlc_settingData.d.results[i].mcs_VIAServiceBaseURL; }
//            if (vlc_settingData.d.results[i].mcs_VIARequestingApplicationCode != null) { vlc_consumingApp = vlc_settingData.d.results[i].mcs_VIARequestingApplicationCode; }
//            if (vlc_settingData.d.results[i].mcs_VIAConsumingApplicationToken != null) { vlc_consumingAppToken = vlc_settingData.d.results[i].mcs_VIAConsumingApplicationToken; }
//            if (vlc_settingData.d.results[i].mcs_VIAConsumingApplicationPassword != null) { vlc_consumingAppPassword = vlc_settingData.d.results[i].mcs_VIAConsumingApplicationPassword; }
//            if (vlc_settingData.d.results[i].mcs_MVISubscriptionKey != null) { vlc_mvisubscriptionkey = vlc_settingData.d.results[i].mcs_MVISubscriptionKey; }
//            break;
//        }
//        if (vlc_DacUrl != null && vlc_ViaLoginApiUrl != null) {
//            //Construct full web service URL
